...
- [8. _EXCEPTION_RECORD](#8-_exception_record)
- [9. _EXCEPTION_REGISTRATION_RECORD](#9-_exception_registration_record)
- [10. VC_EXCEPTION_REGISTRATION](#10-vc_exception_registration)
    - [10.1. scopetable_entry](#101-scopetable_entry)
...
```
# 9. _EXCEPTION_REGISTRATION_RECORD
```x86asm
...
```
# 10. VC_EXCEPTION_REGISTRATION
```x86asm
struct VC_EXCEPTION_REGISTRATION
{
     VC_EXCEPTION_REGISTRATION* prev;        //前一个结构体的指针
     FARPROC                    handler;     //永远指向_exception_handler4回调函数
     scopetable_entry*          scopetable;  //指向scpoetable数组，该数组存储了函数内部所有try-catch结构的相关信息指针
     int                        _index;      //有的书上也叫tryLevel。scopetable项的当前项
     DWORD                      _ebp;        //当前ebp值，用于访问各成员
}
```
## 10.1. scopetable_entry
```x86asm
struct scopetable_entry
{
     DWORD     prev_entryindex;   //上一个scopetable_entry在scopetable中的索引
     FARPROC   lpfnFilter;        //__except块后小括号内的过滤函数的起始地址；__finally时为NULL
     FARPROC   lpfnHandler;       //异常处理程序的地址；__exception或__finally后花括号｛｝内的代码地址
}
```